...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">Novo desconto</h1>
    </div>

    <form action="{{ route('descontos.store') }}" method="POST" class="max-w-3xl space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        @include('descontos._form')
